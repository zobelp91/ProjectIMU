...
def plot3DFrame(s, ax):
    ...
    xn = vl.toVector(0, 1, 0)
    yn = vl.toVector(1, 0, 0)
    zn = vl.toVector(0, 0, -1)

    xb = q.vecTransformation(xn)
    yb = q.vecTransformation(yn)
    zb = q.vecTransformation(zn)

    x1, x2, x3 = vl.toValue(xb)
    y1, y2, y3 = vl.toValue(yb)
    z1, z2, z3 = vl.toValue(zb)
    xb = [x1, x2, x3]
    yb = [y1, y2, y3]
    zb = [z1, z2, z3]
